...
def syncing() -> str:
    ...
    return json.dumps({"syncing": syncing, "height": state.height, "targetHeight": state.target_height})

@api_v1.method(errors=[])
def all_domains(page: int = 1, per_page: int = 100) -> str:
    domains = Domain.select().order_by(Domain.id).limit(per_page).offset((page-1)*per_page)
